...
contract("CoinCrotToken", function(accounts){
    ...
    it('set the total supply', function(){
        return CoinCrotToken.deployed().then(function(instance){
            ...
            assert.equal(totalSuply.toNumber(), 1000000, 'set the total supply');
            return tokenInstance.balanceOf(accounts[0]);
        }).then(function(adminBalance){
            assert.equal(adminBalance.toNumber(), 1000000, 'it allocates the inital supply')
        });
    });
    ...
    it('transfers token ownership', function() {
        return CoinCrotToken.deployed().then(function(instance) {
            ...
            assert.equal(balance.toNumber(), 750000, 'adds the amout to the receiving account');
            return tokenInstance.balanceOf(accounts[1]);
        }).then(function(balance){
            assert.equal(balance.toNumber(), 250000, 'deduct the amount from the sending account');
        })
    });
